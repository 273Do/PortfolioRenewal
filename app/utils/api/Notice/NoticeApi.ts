...
export async function getNoticeAllData() {
  const response = await fetch(
    `https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/notice`,
    {
      cache: "no-store",
    }
  );
...
export async function postNoticeData(value: z.infer<typeof noticeFormSchema>) {
  await fetch(`https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/notice`, {
    method: "POST",
...
export async function updateNoticeData(
  ...
  await fetch(
    `https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/notice/${id}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(value),
