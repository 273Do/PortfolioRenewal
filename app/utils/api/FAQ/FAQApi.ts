...
export async function getFAQData() {
  const allFAQ = await fetch(
    `https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/faq`,
    {
      cache: "no-store",
    }
  );
...
export async function postFAQData(value: z.infer<typeof FAQFormSchema>) {
  await fetch(`https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/faq`, {
    method: "POST",
...
export async function updateFAQData(
  ...
  await fetch(`https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/faq/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(value),
  });
